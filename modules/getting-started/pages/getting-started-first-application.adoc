...
It also provides a xref:using-boot:using-boot-build-systems:using-boot-dependency-management.adoc#using-boot-dependency-management[`dependency-management`] section so that you can omit `version` tags for "`blessed`" dependencies.
...
Spring Boot takes a xref:appendix:executable-jar.adoc#executable-jar[ different approach] and lets you actually nest jars directly.
****
