...
It also provides a xref:spring-boot-reference-documentation:using-boot.adoc#using-boot-dependency-management[`dependency-management`] section so that you can omit `version` tags for "`blessed`" dependencies.
...
Spring Boot takes a xref:spring-boot-reference-documentation:appendix.adoc#executable-jar[ different approach] and lets you actually nest jars directly.
****
