...
If you have not yet developed a Spring Boot web application, you can follow the "Hello World!" example in the _xref:spring-boot-reference-documentation:getting-started.adoc#getting-started-first-application[ Getting started]_ section.
...
For example, it cannot scan for endpoints in a package found in a xref:spring-boot-reference-documentation:deployment.adoc#deployment-install[ fully executable jar file] or in `WEB-INF/classes` when running an executable war file.
...
* xref:spring-boot-reference-documentation:howto.adoc#howto-configure-ssl[SSL]
* xref:spring-boot-reference-documentation:howto.adoc#how-to-enable-http-response-compression[HTTP compression]
...
For instance, xref:spring-boot-reference-documentation:howto.adoc#howto-configure-accesslogs[access logs] can be configured with specific features of the embedded servlet container.
