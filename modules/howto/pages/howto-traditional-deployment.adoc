...
If you use the xref:spring-boot-reference-documentation:build-tool-plugins.adoc#build-tool-plugins[ Spring Boot build tools], marking the embedded servlet container dependency as provided produces an executable war file with the provided dependencies packaged in a `lib-provided` directory.
