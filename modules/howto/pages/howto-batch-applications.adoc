...
====
Unlike command line option arguments that xref:spring-boot-reference-documentation:boot-features.adoc#boot-features-external-config-command-line-args[set properties in the `Environment`] (i.e. by starting with `--`, such as `--my-property=value`), job parameters have to be specified on the command line without dashes (e.g. `jobParam=value`).
====
