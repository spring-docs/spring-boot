...
Note that, thanks to the use of xref:boot-features:boot-features-external-config.adoc#boot-features-external-config-relaxed-binding[ relaxed binding], the case of `indent_output` does not have to match the case of the corresponding enum constant, which is `INDENT_OUTPUT`.
